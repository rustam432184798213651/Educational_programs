...
export default function SearchAndDeleteTab({program, discipline, labWork, value, handler, exportToDocx, exportToPdf}) {
    return (
        <div id="SearchAndDeleteTab">
          <div style={{float: "left", color:"white", fontFamily: "inherit"}}>DB</div> <div style={{float: "right", height: "100%"}}> <DeleteSign /> <PlusSign /> <UpdateSign program={program} discipline={discipline} labWork={labWork} newHtmlContent={value} handler={handler}/> <ExportToDocx  exportToDocx={exportToDocx}/> <ExportToPdf exportToPdf={exportToPdf}/> <RenameSign/> </div>
        </div>
    )
}